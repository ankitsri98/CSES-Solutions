...
ll lis(vector<ll>& nums) {
        ...
        for(int i=1;i<nums.size();i++){
            if(nums[i]>v.back()){
                ...
                ll idx=upper_bound(v.begin(),v.end(),nums[i])-v.begin();
               // cout<<idx<<" "<<i<<endl;
                if(idx<v.size()){
                    v[idx]=nums[i];
                }
...
void solve(){
//so basically in this ques. LIS N*LOG(N) is calculated because this method contains
//elements with smallest values and question asks towers with smallest top elem.
//[ 3,8 ,2,1,5]
//2 towers [3,2,1] [ 8,5];
//LIS [1,5] containing tops of towers so 2 towers
// [3]->[3,8]->[2,8]->[1,8]->[1,5]
    cin>>n;
    vector<ll > arr(n);
    REP(i,0,n)cin>>arr[i];
